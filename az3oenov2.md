...
### 4.3 Arquitectura Funcional Detallada

**Estructura Modular del Sistema:**

El Módulo I está diseñado con una **arquitectura de microservicios** que separa claramente las responsabilidades:

**Capa de Procesamiento de Imágenes:**
Esta capa es responsable de transformar las fotografías de etiquetas en representaciones digitales optimizadas. Cuando un usuario envía una imagen, el sistema:

1. **Recibe y valida** la imagen (formatos JPG, PNG, WEBP)
2. **Aplica preprocesamiento** para mejorar la calidad:
   - Corrección automática de brillo y contraste
   - Eliminación de ruido usando filtros bilaterales
   - Conversión de espacios de color para optimización
   - Redimensionamiento inteligente manteniendo proporciones
3. **Normaliza el formato** a un estándar de 224x224 píxeles que es óptimo para el modelo CLIP

**Capa de Vectorización Semántica:**
Esta es la innovación clave del sistema. Utiliza el modelo **CLIP** de OpenAI para convertir cada imagen en un vector numérico de 512 dimensiones que captura el "significado visual" de la etiqueta:

- **¿Qué captura?** Colores dominantes, tipografía, disposición del texto, formas geométricas, elementos decorativos
- **¿Cómo funciona?** CLIP ha sido entrenado con millones de pares imagen-texto, por lo que "entiende" conceptos visuales
- **Ventaja clave:** No necesita entrenamiento específico para vinos, ya generaliza conocimiento visual

**Motor de Búsqueda Vectorial (FAISS):**
FAISS (Facebook AI Similarity Search) es una librería optimizada que permite buscar entre millones de vectores en milisegundos:

1. **Índice optimizado:** Organiza todos los vectores de vinos en estructuras de datos eficientes
2. **Búsqueda por similitud:** Encuentra los vectores más cercanos usando distancia coseno
3. **Filtrado por umbral:** Solo devuelve resultados con similaridad >70% para garantizar calidad
4. **Escalabilidad:** Puede manejar hasta 10 millones de vinos sin degradación de rendimiento

### 4.4 Sistema OCR Especializado

**Motor de Reconocimiento de Texto Avanzado:**

El sistema OCR está específicamente optimizado para etiquetas de vino en español y desarrolla inteligencia específica del dominio:

**Extracción de Texto Multinivel:**
1. **Nivel básico:** Detección de caracteres usando EasyOCR con soporte para español e inglés
2. **Nivel de confianza:** Filtra automáticamente texto con confianza <50% para evitar errores
3. **Nivel estructurado:** Identifica información específica de vinos usando patrones:
   - **Años:** Reconoce añadas (1950-2025)
   - **Graduación alcohólica:** Detecta porcentajes (ej: "13.5% vol")
   - **Volumen:** Identifica capacidad (750ml, 1.5L, etc.)
   - **Denominaciones:** Reconoce D.O., DOP, IGP automáticamente

**Procesamiento de Lenguaje Natural:**
Integra spaCy para análisis semántico avanzado:
- **Extracción de entidades:** Identifica nombres de bodegas, personas, lugares
- **Análisis contextual:** Entiende relaciones entre términos en la etiqueta
- **Validación cruzada:** Confirma coherencia entre texto extraído e imagen

### 4.5 Flujo de Procesamiento Completo

**Proceso Paso a Paso Cuando Usuario Envía Imagen:**

**Etapa 1 - Recepción y Validación (0.1 segundos):**
- Sistema recibe imagen vía API REST
- Valida formato y tamaño (máximo 10MB)
- Guarda temporalmente para procesamiento

**Etapa 2 - Preprocesamiento Paralelo (0.3 segundos):**
- **Procesamiento visual:** Optimiza imagen para reconocimiento
- **Procesamiento OCR:** Extrae texto en paralelo
- Ambos procesos funcionan simultáneamente para máxima velocidad

**Etapa 3 - Vectorización (0.4 segundos):**
- CLIP convierte imagen optimizada en vector de 512 dimensiones
- Vector se normaliza para búsqueda por similitud coseno
- Se prepara query vector para búsqueda en índice

**Etapa 4 - Búsqueda y Ranking (0.2 segundos):**
- FAISS busca en índice de vinos precomputado
- Encuentra top-10 candidatos por similitud visual
- Aplica filtros de calidad (umbral 70% similaridad)
- Rankea resultados por score de confianza

**Etapa 5 - Enriquecimiento y Respuesta (0.1 segundos):**
- Combina resultados visuales con datos OCR
- Enriquece con metadatos de vino (precio, disponibilidad)
- Formatea respuesta JSON estructurada
- **Tiempo total: <1.5 segundos**

### 4.6 Búsqueda Textual Semántica

**Capacidad Multimodal Única:**

El sistema permite búsquedas textuales usando el mismo motor de vectorización, creando una experiencia unificada:

**¿Cómo funciona la búsqueda textual?**
1. **Usuario escribe:** "vino blanco seco de Rueda para pescado"
2. **CLIP procesa texto:** Convierte frase en vector semántico de 512D
3. **Búsqueda cruzada:** Compara vector de texto contra vectores de imágenes
4. **Resultados inteligentes:** Encuentra vinos que "visualmente corresponden" al concepto textual

**Ventajas de la Búsqueda Semántica:**
- **No literal:** "vino elegante" encuentra vinos premium sin esa palabra exacta
- **Contextual:** "para cena romántica" entiende el contexto y sugiere vinos apropiados
- **Multiidioma:** Funciona en español e inglés automáticamente
- **Evolutiva:** Mejora con uso y feedback de usuarios

### 4.7 Proceso de Construcción del Índice

**Preparación Inicial del Sistema:**

Antes de que el sistema pueda reconocer vinos, debe construirse un índice maestro:

**Fase 1 - Preparación de Datos:**
- Se organiza catálogo de AZ3OENO en formato estructurado CSV
- Cada vino tiene: ID único, nombre, bodega, imagen, metadatos
- Se valida calidad de imágenes (resolución mínima, formato correcto)

**Fase 2 - Procesamiento Masivo:**
- Sistema procesa automáticamente todas las imágenes del catálogo
- Para cada imagen: preprocesamiento → vectorización CLIP → almacenamiento
- Proceso batch optimizado: ~1000 imágenes por hora en hardware estándar

**Fase 3 - Construcción del Índice:**
- FAISS organiza todos los vectores en estructura optimizada
- Crea índices secundarios para filtrado rápido
- Valida integridad y calidad del índice final

**Fase 4 - Optimización:**
- Sistema ajusta parámetros según características del catálogo
- Configura umbrales de similitud específicos para vinos
- Implementa caché para consultas frecuentes

### 4.8 Configuración y Adaptabilidad

**Sistema Altamente Configurable:**

La arquitectura permite adaptación a diferentes necesidades de AZ3OENO:

**Configuración de Modelos:**
- **Modelo CLIP:** Seleccionable entre variantes según precisión vs velocidad
- **Idiomas OCR:** Expandible a francés, italiano, portugués
- **Umbrales:** Ajustables según calidad deseada vs cobertura

**Configuración de Rendimiento:**
- **Escalado automático:** Se adapta a carga de usuarios
- **Caché inteligente:** Optimiza respuestas para consultas frecuentes
- **Balanceadores:** Distribuye carga entre múltiples instancias

**Configuración de Negocio:**
- **Filtros personalizados:** Por precio, disponibilidad, región
- **Integración inventario:** Conexión en tiempo real con stock
- **Analytics:** Tracking detallado de uso y rendimiento

El sistema está diseñado para **evolucionar con AZ3OENO**, añadiendo nuevas capacidades sin interrumpir el servicio existente.

---

### 5.3 Arquitectura del Knowledge Graph Detallada

**Diseño del Grafo de Conocimiento de Vinos:**

El Knowledge Graph es el cerebro estructurado del sistema de recomendación. Representa el conocimiento del mundo del vino como una red interconectada de entidades y relaciones semánticas.

**Entidades Principales del Grafo:**

**Nodo Central - Vino:**
Cada vino es un nodo central que se conecta con múltiples tipos de información:
- **Propiedades intrínsecas:** Nombre, añada, graduación alcohólica, acidez, taninos
- **Propiedades de producción:** Tiempo de crianza, tipo de barrica, técnicas de vinificación
- **Propiedades comerciales:** Precio, disponibilidad, puntuaciones de críticos

**Nodos de Contexto:**
- **Bodega:** Información del productor, historia, filosofía, región de ubicación
- **Variedad de Uva:** Características organolépticas, regiones típicas, estilos de vinificación
- **Denominación de Origen:** Regulaciones, clima, terroir, tradiciones vitivinícolas
- **Maridaje:** Tipos de comida, ocasiones, momentos del día apropiados
- **Aromas y Sabores:** Descriptores sensoriales organizados por familias aromáticas

**Sistema de Relaciones Semánticas:**

**Relaciones de Producción:**
- `PRODUCIDO_POR`: Conecta vino con bodega específica
- `ELABORADO_CON`: Relaciona vino con varietales utilizados (puede ser múltiple)
- `PERTENECE_A`: Vincula vino con denominación de origen
- `CRIADO_EN`: Especifica tipo de crianza (barrica francesa, americana, acero, etc.)

**Relaciones de Características:**
- `TIENE_AROMA`: Conecta con descriptores aromáticos específicos
- `PRESENTA_SABOR`: Relaciona con notas gustativas
- `NIVEL_TANINOS`: Especifica intensidad (bajo, medio, alto)
- `GRADO_ACIDEZ`: Define perfil ácido del vino

**Relaciones de Consumo:**
- `MARIDA_CON`: Conecta con tipos de comida específicos
- `APROPIADO_PARA`: Relaciona con ocasiones (cena formal, aperitivo casual)
- `TEMPERATURA_SERVICIO`: Especifica rango de temperatura óptimo
- `TIEMPO_GUARDA`: Indica potencial de evolución en botella

### 5.4 Motor de Razonamiento Híbrido

**Integración Inteligente de Embeddings y Grafo:**

El sistema combina la flexibilidad de los embeddings semánticos con la precisión del conocimiento estructurado del grafo:

**Proceso de Recomendación Paso a Paso:**

**Etapa 1 - Comprensión de la Consulta:**
Cuando el usuario dice: "Busco un vino tinto elegante con notas de vainilla para acompañar cordero asado"

1. **Análisis semántico:** El motor de embeddings identifica conceptos clave
   - "tinto elegante" → vector semántico que representa sofisticación
   - "notas de vainilla" → vector que captura descriptores aromáticos
   - "cordero asado" → vector de contexto gastronómico

2. **Estructuración de la consulta:** El Knowledge Graph interpreta:
   - Tipo: vino_tinto
   - Perfil aromático: vainilla (indica crianza en barrica)
   - Maridaje: carne_roja_asada
   - Ocasión: cena_formal (implícito en "elegante")

**Etapa 2 - Búsqueda Híbrida:**

**Candidatos por Similitud Semántica:**
- El motor de embeddings encuentra vinos cuyas descripciones son semánticamente similares
- Utiliza ChromaDB para búsqueda vectorial eficiente
- Obtiene top-20 candidatos con scores de similitud

**Filtrado por Knowledge Graph:**
- Aplica filtros estructurales: tipo = "tinto"
- Busca vinos conectados a nodo "vainilla" en aromas
- Filtra por conexiones a "carne_roja" en maridajes
- Verifica relaciones con "crianza_barrica" (fuente típica de vainilla)

**Etapa 3 - Scoring Combinado:**
Score_Final = (0.6 × Score_Semántico) + (0.4 × Score_Grafo)

- **Score Semántico:** Similitud coseno entre consulta y descripción del vino
- **Score Grafo:** Número y calidad de conexiones relevantes en el Knowledge Graph

### 5.5 Generador de Explicaciones Inteligentes

**Motor de Razonamiento Explicativo:**

Una de las ventajas únicas del sistema es su capacidad para explicar por qué recomienda cada vino específico:

**Análisis de Caminos en el Grafo:**
El sistema rastrea las conexiones que justifican cada recomendación:

**Ejemplo de Razonamiento:**
Para el vino "Viña Ardanza Reserva 2016":

1. **Conexión directa:** Vino → TIENE_AROMA → Vainilla ✓
2. **Conexión de maridaje:** Vino → MARIDA_CON → Carne_Roja ✓  
3. **Conexión de estilo:** Vino → CRIANZA_EN → Barrica_Francesa → APORTA → Vainilla ✓
4. **Conexión varietal:** Vino → ELABORADO_CON → Tempranillo → CARACTERÍSTICA → Elegante ✓

**Generación de Explicación en Lenguaje Natural:**
El sistema convierte estas conexiones en explicaciones comprensibles:

"Te recomendamos el Viña Ardanza Reserva 2016 porque presenta notas de vainilla debido a su crianza en barrica francesa, está elaborado con Tempranillo que le aporta elegancia natural, y su perfil con cuerpo medio hace que maride perfectamente con cordero asado."

### 5.6 Sistema Conversacional con LLM

**Integración Avanzada con Large Language Models:**

El sistema utiliza GPT-4 o Claude para crear una experiencia conversacional natural que va más allá de búsquedas simples:

**Gestión de Contexto Conversacional:**

**Memoria de Sesión Inteligente:**
- Mantiene historial de preferencias expresadas durante la conversación
- Recuerda vinos mencionados o rechazados por el usuario
- Aprende patrones de preferencias para refinamiento progresivo

**Extracción de Preferencias Implícitas:**
El LLM identifica información no explícita en las consultas:

- Usuario: "Quiero algo para una cena romántica"
  - **Implícito detectado:** Ocasión especial → vino premium, ambiente íntimo → vino elegante, cena para dos → botella estándar

- Usuario: "Me gustó el Albariño del otro día pero busco algo más seco"
  - **Implícito detectado:** Prefiere blancos, conoce Albariño → puede gustarle Godello, "más seco" → menor azúcar residual

**Diálogo Guiado y Educativo:**
El sistema no solo recomienda, sino que educa al usuario:

**Ejemplo de Conversación Educativa:**
- Usuario: "¿Por qué me recomiendas un Godello si pedí algo como Albariño?"
- Sistema: "El Godello es de la misma región que algunos Albariños (Rías Baixas), pero tiene menos azúcar residual y mayor mineralidad. Como mencionaste que querías algo 'más seco', el Godello te dará esa sensación de frescura que buscas, pero con un perfil más complejo y menos floral que el Albariño típico."

### 5.7 Proceso de Construcción y Enriquecimiento del Grafo

**Construcción Automatizada del Knowledge Graph:**

**Fase 1 - Carga de Datos Base:**
- Importación del catálogo de vinos de AZ3OENO con metadatos estructurados
- Creación automática de nodos para cada vino, bodega, variedad
- Establecimiento de relaciones básicas (vino-bodega, vino-D.O.)

**Fase 2 - Enriquecimiento Semántico:**
- Análisis de descripciones textuales para extraer aromas y sabores
- Creación automática de nodos de maridaje basados en recomendaciones expertas
- Conexión con bases de datos externas (clima, terroir, premios)

**Fase 3 - Aprendizaje Continuo:**
- El sistema aprende de las consultas de usuarios para identificar nuevas relaciones
- Cuando usuarios buscan "vino frutal para barbacoa", el sistema refuerza la conexión vino_frutal ↔ carne_a_la_parrilla
- Análisis de feedback de usuarios para validar y ajustar recomendaciones

**Validación y Calidad del Grafo:**
- **Consistencia semántica:** Verificación automática de relaciones contradictorias
- **Completitud:** Identificación de nodos con pocas conexiones para enriquecimiento
- **Actualización:** Incorporación de nuevos vinos y tendencias del mercado

### 5.8 Arquitectura de Despliegue y Escalabilidad

**Diseño para Alto Rendimiento:**

**Distribución de Componentes:**
- **Neo4j Cluster:** Para el Knowledge Graph con replicación para alta disponibilidad
- **ChromaDB Distribuido:** Para búsquedas vectoriales escalables
- **Caché Redis:** Para consultas frecuentes y sesiones de usuario
- **Load Balancer:** Para distribución inteligente de carga entre instancias

**Optimizaciones de Rendimiento:**

**Caché Inteligente Multi-Nivel:**
1. **Caché L1 (memoria):** Consultas más frecuentes en RAM
2. **Caché L2 (Redis):** Resultados de recomendaciones complejas  
3. **Caché L3 (disco):** Vectores precomputados y subgrafos

**Precomputación Estratégica:**
- Vectores de consultas populares se precomputan y almacenan
- Subgrafos frecuentes se mantienen en memoria para acceso rápido
- Rankings de vinos por categorías se actualizan periódicamente

**Monitoreo y Analytics:**
- **Métricas de rendimiento:** Tiempo de respuesta, throughput, calidad de recomendaciones
- **Análisis de uso:** Patrones de consulta, preferencias de usuarios, tendencias de mercado
- **Alertas inteligentes:** Detección automática de degradación de calidad o problemas de rendimiento

El sistema está diseñado para **crecer orgánicamente** con AZ3OENO, desde un catálogo inicial de miles de vinos hasta potencialmente millones, manteniendo siempre alta calidad y velocidad de respuesta.### 5.9 Ejemplo Completo de Flujo de Recomendación

**Caso de Uso Real: Usuario Busca Vino para Cena Especial**

**Consulta Usuario:** "Busco un vino tinto elegante con notas afrutadas para una cena romántica con mi pareja. Vamos a cenar salmón a la plancha con salsa holandesa. Presupuesto máximo 35€."

**Proceso Interno Paso a Paso:**

**Etapa 1 - Análisis de la Consulta (0.2 segundos):**
El sistema LLM descompone la consulta identificando:
- **Tipo de vino:** Tinto
- **Perfil gustativo:** Elegante, afrutado
- **Contexto:** Cena romántica (ocasión especial)
- **Maridaje:** Salmón + salsa holandesa (pescado graso)
- **Restricción:** Presupuesto ≤35€

**Etapa 2 - Búsqueda Semántica (0.3 segundos):**
El motor de embeddings procesa la consulta textual:
- Convierte "tinto elegante afrutado" en vector semántico
- Busca en ChromaDB vinos con descripciones similares
- Obtiene 15 candidatos iniciales con scores >0.75

**Etapa 3 - Filtrado por Knowledge Graph (0.2 segundos):**
El sistema consulta el grafo para refinar resultados:
- Filtra por tipo = "tinto"
- Busca conexiones a nodos "frutal" en aromas
- Identifica vinos apropiados para "pescado_graso" (insight del grafo)
- Aplica filtro de precio ≤35€

**Etapa 4 - Scoring Híbrido (0.1 segundos):**
Combina puntuaciones de ambos sistemas:
- Score semántico: similitud con descripción textual
- Score grafo: número de conexiones relevantes
- Peso combinado: 60% semántico + 40% grafo

**Etapa 5 - Generación de Explicaciones (0.2 segundos):**
Para cada recomendación, el sistema rastrea las conexiones que justifican la sugerencia:

**Resultado Final (Total: 1.0 segundos):**

**1. Viña Pomal Centenario Crianza 2019 - Score: 0.91**
"Te recomendamos este vino porque presenta notas afrutadas intensas de grosella y cereza, su elegancia proviene de 12 meses en barrica francesa, y aunque es tinto, tiene la acidez y suavidad perfecta para acompañar salmón sin enmascarar su sabor delicado. Precio: 28€"

**2. Marqués de Cáceres Reserva 2018 - Score: 0.87**
"Este Reserva combina elegancia y fruta madura con un perfil menos tánico ideal para pescado. Su crianza aporta complejidad sin ser demasiado potente. La acidez equilibrada complementa la grasa natural del salmón. Precio: 32€"

**3. Cune Crianza 2020 - Score: 0.83**
"Un tinto versátil con notas de frutos rojos y final suave. Su perfil elegante pero accesible lo hace perfecto para una cena romántica, y sorprendentemente funciona bien con pescados grasos por su estructura equilibrada. Precio: 18€"

### 5.10 Capacidades Avanzadas del Sistema

**Aprendizaje Contextual Dinámico:**

**Adaptación a Feedback del Usuario:**
Cuando el usuario responde "Me gusta la primera opción pero prefiero algo un poco más suave":

1. **El sistema registra la preferencia:** Viña Pomal ✓ pero buscar menos taninos
2. **Actualiza el perfil temporal:** Usuario prefiere elegancia con suavidad
3. **Re-ejecuta búsqueda:** Mismos criterios pero con filtro adicional "taninos_bajos"
4. **Nueva recomendación:** Busca vinos similares pero de varietales naturalmente más suaves

**Memoria de Preferencias por Sesión:**
El sistema mantiene contexto durante toda la conversación:
- Vinos mencionados y evaluación del usuario
- Preferencias expresadas directa o indirectamente  
- Contexto de la ocasión y restricciones

**Educación Progresiva del Usuario:**
El sistema no solo recomienda, sino que educa sutilmente:

"*Dato curioso: Aunque recomendamos tintos para tu salmón, esto se debe a que el salmón tiene más grasa que otros pescados, lo que permite maridar con tintos ligeros y afrutados como estos. La regla tradicional 'pescado-blanco' se puede romper cuando entendemos los perfiles de sabor.*"

### 5.11 Integración con Módulo I (Reconocimiento)

**Sinergia entre Sistemas:**

Cuando un usuario combina ambos módulos, el sistema crea una experiencia fluida y coherente:

**Caso de Uso Integrado:**
1. **Usuario fotografía etiqueta** en restaurante → Módulo I identifica "Ribera del Duero Crianza 2020"
2. **Usuario pregunta:** "Me gustó este vino, ¿qué me recomiendas similar para comprar?"
3. **Sistema usa datos del Módulo I** para alimentar recomendación del Módulo II
4. **Consulta interna generada:** "vino similar a Ribera del Duero Crianza 2020, Tempranillo, taninos medios, para compra doméstica"

**Enriquecimiento Cruzado:**
- OCR del Módulo I enriquece Knowledge Graph del Módulo II
- Preferencias del Módulo II mejoran precisión del Módulo I
- Historial integrado crea perfil completo del usuario

### 5.12 Métricas de Calidad y Optimización

**Sistema de Evaluación Continua:**

**Métricas de Precisión:**
- **Relevancia de recomendaciones:** % usuarios que consideran sugerencias apropiadas
- **Satisfacción explicativa:** % usuarios que entienden el "por qué" de la recomendación
- **Conversión a compra:** % recomendaciones que resultan en compra real

**Métricas de Rendimiento:**
- **Tiempo de respuesta end-to-end:** <1.5 segundos objetivo
- **Escalabilidad:** Capacidad de manejar consultas concurrentes
- **Precisión semántica:** Alineación entre consulta del usuario y resultados

**Optimización Continua:**
- **A/B Testing:** Diferentes pesos entre semántica y grafo según tipo de consulta
- **Feedback Loop:** Incorporación de evaluaciones de usuario para mejorar algoritmos
- **Reentrenamiento:** Actualización periódica de embeddings con nuevos datos

**Personalización Adaptativa:**
El sistema aprende patrones individuales:
- Usuarios que prefieren explicaciones técnicas vs simples
- Preferencias estacionales (frescos en verano, corpulentos en invierno)
- Sensibilidad al precio y tendencia a aceptar sugerencias premium

### 5.13 Escalabilidad y Evolución del Sistema

**Crecimiento Orgánico del Knowledge Graph:**

**Incorporación Automática de Nuevos Vinos:**
Cuando AZ3OENO añade nuevos vinos al catálogo:
1. **Análisis automático** de descripción y características
2. **Identificación de entidades** (bodega, variedad, D.O.)
3. **Creación de conexiones** basadas en similitudes detectadas
4. **Validación cruzada** con vinos existentes para consistencia

**Expansión Geográfica:**
El sistema está diseñado para crecer más allá del mercado español:
- **Modelos multiidioma:** Soporte para francés, italiano, portugués
- **Conocimiento regional:** Incorporación de D.O. y tradiciones locales
- **Adaptación cultural:** Diferentes preferencias de maridaje por país

**Integración con Nuevas Fuentes de Datos:**
- **APIs de críticos de vino:** Incorporación de puntuaciones profesionales
- **Datos climáticos:** Información de añadas y condiciones meteorológicas
- **Tendencias de mercado:** Análisis de popularidad y emergencia de nuevos estilos

El sistema de recomendación de AZ3OENO representa un **avance cualitativo** en wine-tech, combinando la **potencia de la AI moderna** con el **conocimiento profundo del dominio vitivinícola** para crear experiencias de descubrimiento de vino únicas y personalizadas.
