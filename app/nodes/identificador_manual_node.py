...
Responde solo con "modificar" o "continuar".
    ...
    async def execute(self, state: EroskiState) -> Command:
        logging.info("👹🏅🏅RecogerDatosEmpleadoNode simplificado con control de intentos🏅🏅👹")
        ...
        intento_identificacion = state.get("intento_identificacion", 0)
        max_intentos_identificacion = state.get("max_intentos_identificacion")
        
        if intento_identificacion >= max_intentos_identificacion:
            return self._supervisor_node("Lo siento, no puedo identificar a este usuario,\nTe paso con un supervisor.")
        else:
            intento_identificacion += 1
            state["intento_identificacion"] = intento_identificacion
        ...
        if isinstance(datos_extraidos, Command):
            print("👹👹👹 datos_extraidos es un comando👹👹👹")
    ...
    async def execute_kk2(self, state: EroskiState) -> Command:
        logging.info("👹🏅🏅RecogerDatosEmpleadoNode🏅🏅👹")
        messages = state.get("messages", [])
        ultimo_msg = next((m for m in reversed(messages) if isinstance(m, HumanMessage)), None)
        if not ultimo_msg:
            return self._respuesta_ai("Hola, ¿cómo te llamas?")
        ...
        if state.get("modificaciones_pendientes"):
            logging.info(f"modifaciones: {state.get('modificaciones_pendientes')}")
            return self._pending_confirmation(state, mensaje_usuario)
        ...
        nuevo_estado["intento_identificacion"] = intento_identificacion + 1

        if isinstance(datos_extraidos, Command):
            return datos_extraidos


        logging.info(f"👹datos_extraidos: {datos_extraidos}")
        resultado_confirmacion = self._detectar_cambios(state, datos_extraidos)
        logging.info(f"👹resultado_confirmacion: {resultado_confirmacion}")
    ...
    async def execute_kk(self, state: EroskiState) -> Command:
        ...
        if intencion == "modificar":
            ...
            campos = ["incident_user_name", "incident_last_name", "incident_store_name", "incident_department"]
            for campo in campos:
                logging.info(f"👹campo: {campo} - {nuevo_estado.get(campo, 'No especificado')}")
    ...
    def _supervisor_node(self, texto: str) -> Command:
    ...
    def _llm_extraccion(self, datos_actuales: Dict[str, Optional[str]], 
        ...
        kk = f"""
Eres un asistente de soporte interno de Eroski. Tu tarea es extraer información del mensaje del usuario y devolver los siguientes campos estructurados:

Campos a extraer (máximo 4):
- nombre
- apellido
- seccion (ej. Carnicería, Pescadería, Panadería, Caja, etc.)
- tienda (de esta lista: {', '.join(self.tiendas[:20])})

Si identificas una tienda que no está en la lista, guárdala en `tienda_tentativa`.

Marca `authenticated` como `true` **solo** si tienes los 4 campos correctamente identificados y válidos (incluida la tienda en la lista o aceptada como tentativa).
...
- Solo extrae o modifica un campo si el usuario lo menciona de forma clara y directa.
- Si el usuario da un dato distinto al actual, considera que desea actualizarlo.
- Si no se menciona un campo, devuélvelo como `null`.
- No hagas inferencias. Por ejemplo, si dice "problema con el TPV", **no** asumas que es sección Caja.

---

INSTRUCCIONES PARA LA RESPUESTA AL USUARIO (campo `respuesta`):
- Si actualizas algún dato respecto al estado anterior, comunícalo claramente.
  Ejemplo: "He actualizado tu sección a Panadería."
- Si ya tienes los 4 campos, da las gracias, resume los datos recogidos y avisa que pasarás a gestionar la incidencia.
- **Siempre pide amablemente los datos que aún falten**: nombre, apellido, sección o tienda.
- Recuerda pedir los campos que falten, ya sean uno o varios. Por ejemplo, si te falta la tienda y el apellido, pregunta por la tienda y el apellido.
- Si la tienda indicada no está en la lista:
  - Si `intento_tienda` es 1 o 2 → pide amablemente que vuelva a indicar la tienda.
  - Si `intento_tienda` es 3 → guarda esa tienda como `tienda_tentativa`, avisa al usuario que no la encuentras pero la usarás igualmente, y copia su valor también en el campo `tienda`.
- Si `tienda_tentativa` ya tiene un valor:
  - Y el usuario vuelve a escribir **la misma tienda** que no está en la lista → indícale que ya intentaste identificar esa tienda y que no figura como válida.
  - Y el usuario proporciona otra tienda también no válida → avísale que esta nueva tampoco figura en la lista.
...
Devuelve un objeto con esta estructura exacta:
...
{{{{
  ...
  "tienda_tentativa": <string o null>,
  "authenticated": <true o false>,
  "respuesta": <texto para el usuario>
}}}}

No incluyas ningún texto fuera del JSON. Tu única salida debe ser el JSON.
"""
